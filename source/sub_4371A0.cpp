//----- (004371A0) --------------------------------------------------------
extern char asc_4AA198[];

char __cdecl sub_4371A0(int self, char a2)
{
  char v2; // bl
  char v4; // al
  int v5; // eax
  int v6; // ecx
  const char *v7; // eax
  int v8; // eax
  _DWORD *v9; // ecx
  char *v10; // edi
  int v11; // ecx
  const char *v12; // eax
  int v13; // eax
  LPVOID *v14; // eax
  int v16; // [esp+Ch] [ebp-104h]
  char Buffer[256]; // [esp+10h] [ebp-100h] BYREF

  v2 = a2;
  *(_BYTE *)&v16 = a2;
  if ( isupper(a2) )
  {
    v2 = tolower(a2);
    *(_BYTE *)&v16 = v2;
  }
  v4 = *(_BYTE *)(self + 724);
  if ( v4 )
  {
    if ( v2 == 13 )
    {
      *(_BYTE *)(dword_520970 + 296) = 0;
      v9 = *(_DWORD **)(self + 736);
      *(_BYTE *)(self + 724) = 0;
      sub_421280(v9);
      v10 = *(char **)(*(_DWORD *)(self + 736) + 8);
      if ( *(_BYTE *)(dword_4F5CC4 + 936) || *(_BYTE *)(dword_4F5CC4 + 937) )
      {
        sub_463710((const char *)*(_DWORD *)(*(_DWORD *)(self + 736) + 8));
      }
      else
      {
        v11 = *(_DWORD *)(dword_520970 + 280);
        v12 = (const char *)(v11 + 472);
        if ( !*(_BYTE *)(v11 + 472) )
        {
          v13 = *(_DWORD *)(v11 + 512);
          if ( v13 <= -1 )
            v12 = AppName;
          else
            v12 = (const char *)(352 * v13 + v11 + 899);
        }
        sprintf(Buffer, "%s: %s", v12, *(const char **)(*(_DWORD *)(self + 736) + 8));
        v14 = (LPVOID *)sub_436A90(self);
        sub_421110(v14, Buffer);
        if ( !*(_BYTE *)(dword_4F5CC4 + 936) && !*(_BYTE *)(dword_4F5CC4 + 937) )
          sub_4375C0((void *)self, v10);
      }
      return sub_4285C0((_DWORD *)self, v16);
    }
  }
  else if ( v2 == 13 )
  {
    if ( !*(_BYTE *)(dword_520970 + 296) && !(unsigned __int8)sub_450BB0((_DWORD *)dword_520970) )
    {
      *(_BYTE *)(self + 724) = 1;
      v5 = dword_520970;
      if ( !*(_BYTE *)(dword_520970 + 296) )
      {
        *(_BYTE *)(dword_520970 + 296) = 1;
        v5 = dword_520970;
      }
      v6 = *(_DWORD *)(v5 + 280);
      v7 = (const char *)(v6 + 472);
      if ( !*(_BYTE *)(v6 + 472) )
      {
        v8 = *(_DWORD *)(v6 + 512);
        if ( v8 <= -1 )
          v7 = AppName;
        else
          v7 = (const char *)(352 * v8 + v6 + 899);
      }
      sub_421190(*(_DWORD *)(self + 732), "%s: ", v7);
      sub_421110(*(LPVOID **)(self + 736), asc_4AA198);
    }
    return sub_4285C0((_DWORD *)self, v16);
  }
  if ( v2 == 27 )
  {
    *(_BYTE *)(self + 724) = 0;
  }
  else if ( v4 )
  {
    if ( isprint(a2) && strlen(*(const char **)(*(_DWORD *)(self + 736) + 8)) < 0x3C )
    {
      sub_421280(*(_DWORD **)(self + 736));
      sub_4211E0(*(const char ***)(self + 736), a2);
      sub_4211E0(*(const char ***)(self + 736), 95);
      if ( *(float *)(*(_DWORD *)(self + 732) + 24) + *(float *)(*(_DWORD *)(self + 732) + 24) < 512.0 )
        return sub_4285C0((_DWORD *)self, v16);
    }
    else if ( v2 != 8 )
    {
      return sub_4285C0((_DWORD *)self, v16);
    }
    sub_421280(*(_DWORD **)(self + 736));
    sub_421280(*(_DWORD **)(self + 736));
    sub_4211E0(*(const char ***)(self + 736), 95);
  }
  return sub_4285C0((_DWORD *)self, v16);
}
